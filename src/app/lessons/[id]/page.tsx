...
function stripEmoji(text: string) {
  return text.replace(/^[\u{1F300}-\u{1FFFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]\s*/u, "").trim();
}
 ...
 * Each word like "Descendre" is split off from the bold letter that follows in the surrounding text.
 * We only need the string itself here; the surrounding prose has the expansions.
 */
function parseMnemoLetters(text: string): string[] {
  return text.trim().split(/[\s·]+/).filter(Boolean);
}

// Detect `li` whose first child is a `strong` (chip-style items)
function getChipContent(node: any): { label: string; rest: any[] } | null {
  const children = node?.children || [];
  if (children.length === 0) return null;
  const first = children[0];
  // first child is a strong element
  if (first?.type === "element" && first?.tagName === "strong") {
    const label = first.children?.map((c: any) => c.value || "").join("") || "";
    return { label, rest: children.slice(1) };
  }
  return null;
}
...
export default function LessonDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  // section progress: track which h2 sections have been scrolled past
  const [sectionsTotal] = useState(3); // Théorie / Exemples / Astuce
  ...
  // Reading progress + section detection
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // section progress based on scroll thirds
      if (progress < 30) setCurrentSection(0);
      else if (progress < 70) setCurrentSection(1);
  ...
  // ── markdown state (reset per render, stable across re-renders via closure) ──
  ...
  const markdownComponents = {
    ...
    li: ({ children, node, ordered }: any) => {
      ...
      // ── chip items : **Label** : contenu ─────────────────────────────────
      const chip = getChipContent(node);
      if (chip) {
        return (
          <li className="flex items-start gap-3 py-1">
            <span className="mt-0.5 shrink-0 px-3 py-1 rounded-xl bg-indigo-100 text-indigo-700 text-[11px] font-black uppercase tracking-wide whitespace-nowrap">
              {chip.label}
            </span>
            <span className="text-slate-600 font-medium leading-relaxed">{children}</span>
          </li>
        );
      }

      // ── ordered list item → dialectic card ───────────────────────────────
      if (ordered) {
        const colors = [
        ...
        // we can't get the real index easily here; cycle via a module-level counter trick
        // instead use a neutral style — still card-shaped
        return (
          <li className="flex items-start gap-3 p-4 rounded-2xl border bg-slate-50 border-slate-200 text-slate-700 font-medium">
            {children}
          </li>
        );
      }
    ...
    // Italic — dialogue bubbles or plain italic
    em: ({ children }: any) => {
    ...
    // strong — detect mnemo (DR MRS VANDERTRAMP style) for badge grid
    strong: ({ children }: any) => {
      const text = children?.toString() || "";
      if (isMnemo(text)) {
  ...
  // ── section progress pill labels ─────────────────────────────────────────
  const sectionLabels = ["Théorie", "Exemples", "Astuce"];
                  ...
                  <div className="flex items-center gap-2 pt-1">
                    {sectionLabels.map((label, i) => (
                      <div key={label} className="flex items-center gap-2">
                        <motion.div
                          animate={{
                            backgroundColor: i <= currentSection ? '#4f46e5' : '#e4e4e7',
                            color: i <= currentSection ? '#ffffff' : '#a1a1aa',
                          }}
                        ...
                        {i < sectionLabels.length - 1 && (
                          <motion.div
                            animate={{ backgroundColor: i < currentSection ? '#4f46e5' : '#e4e4e7' }}
                            transition={{ duration: 0.4 }}
