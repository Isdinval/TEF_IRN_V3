...
import { Search, Filter, Sparkles, Clock, ChevronRight, BookOpen, Mic, FileText, Cpu } from 'lucide-react';
...
export default function GuidesPage() {
  ...
  const levels = useMemo(() => {
    const all = guides.flatMap(g => g.level?.split(',').map(l => l.trim()) || []);
    return Array.from(new Set(all)).filter(Boolean).sort();
  }, [guides]);

  const categories = useMemo(() => {
    const all = guides.map(g => g.category).filter(Boolean);
    return Array.from(new Set(all)) as string[];
  }, [guides]);

  const types = useMemo(() => {
    const all = guides.map(g => g.type).filter(Boolean);
    return Array.from(new Set(all)) as string[];
  }, [guides]);
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] selection:bg-blue-100 pb-24">
      ...
      <section className="bg-white border-b border-gray-100 py-20 px-6 overflow-hidden relative">
        ...
        <div className="max-w-5xl mx-auto text-center space-y-8 relative z-10">
          ...
          <motion.h1
            ...
            className="text-5xl lg:text-7xl font-black tracking-tight text-zinc-900 leading-[1.1]"
          >
            Le savoir est la clé <br />de votre <span className="text-blue-600">réussite</span>.
          </motion.h1>
          ...
          >
            Maîtrisez chaque aspect de l'examen TEF IRN avec nos guides pédagogiques,
            astuces exclusives et méthodologies détaillées.
          </motion.p>
          ...
          >
            <div className="absolute inset-0 bg-blue-600/5 blur-2xl rounded-full group-focus-within:bg-blue-600/10 transition-colors"></div>
            <div className="relative flex items-center bg-white border-2 border-gray-100 rounded-[2rem] p-2 pr-4 shadow-xl shadow-gray-200/50 focus-within:border-blue-600/20 transition-all">
              <div className="pl-4 text-gray-400">
                <Search size={20} />
              </div>
              <Input
                type="text"
                placeholder="Rechercher un guide (ex: expression orale, B1...)"
                className="border-none shadow-none focus-visible:ring-0 text-lg py-6"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
              <div className="hidden sm:flex gap-2">
                <Badge variant="secondary" className="bg-gray-50 text-gray-400 font-bold px-3 py-1 text-[10px] uppercase tracking-tighter">
                  ESC pour annuler
                </Badge>
              </div>
            </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <aside className="w-full lg:w-64 space-y-10 shrink-0">
            ...
            <div className="bg-zinc-900 rounded-[2rem] p-6 text-white overflow-hidden relative group">
              <div className="absolute -top-4 -right-4 w-24 h-24 bg-blue-600 rounded-full blur-3xl opacity-20 group-hover:opacity-40 transition-opacity"></div>
              <p className="text-xs font-black uppercase tracking-widest text-blue-400 mb-4">Besoin d'aide ?</p>
              <h4 className="text-lg font-bold mb-4 leading-tight">Vous ne trouvez pas un guide spécifique ?</h4>
              <Button size="sm" className="w-full bg-white text-zinc-900 hover:bg-gray-100 font-black rounded-xl">
                Contacter un expert
              </Button>
            </div>
          </aside>
          ...
          <div className="flex-grow">
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-2xl font-black text-zinc-900">
                {isLoading ? 'Chargement...' : `${filteredGuides.length} guide${filteredGuides.length > 1 ? 's' : ''} disponible${filteredGuides.length > 1 ? 's' : ''}`}
              </h2>
            </div>
      ...
      {/* Popular Section */}
      {!isLoading && filteredGuides.length > 0 && !searchQuery && !activeLevel && (
