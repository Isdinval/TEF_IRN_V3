...
export default function LessonCard({ lesson, index, isNext, category, parcoursId }: LessonCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        <Card className={`group relative overflow-hidden border-none shadow-sm transition-all duration-300 rounded-3xl border-t-4 ${theme.border} ${
          isCompleted ? 'bg-emerald-50/50' : 'bg-white'
        } ${isNextState ? 'ring-2 ring-indigo-600 ring-offset-4 shadow-lg' : 'hover:shadow-xl'}`}>
          ...
          <CardContent className="p-8 md:p-10 flex flex-col md:flex-row items-center gap-8 md:gap-10 relative z-10">
            ...
            <div className="shrink-0">
              <div className={`w-24 h-24 rounded-3xl flex items-center justify-center text-3xl transition-transform duration-500 ${!isLocked ? 'group-hover:rotate-6' : ''} ${
                isCompleted
                ? 'bg-emerald-100 text-emerald-600'
                : isNextState ? `${theme.button} text-white shadow-lg ${theme.shadow}`
                : isLocked ? 'bg-zinc-100 text-zinc-500'
                : `bg-zinc-50 text-zinc-500 group-hover:bg-opacity-80 ${theme.hoverIconBg}`
              }`}>
                {isCompleted ? (
            ...
            <div className="flex-1 text-center md:text-left space-y-4">
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                {isCompleted ? (
                  <Badge className="bg-emerald-100 text-emerald-700 rounded-full px-4 py-1 text-xs font-black uppercase tracking-wider border-none">
                    Terminé
                  </Badge>
                ) : isNextState ? (
                  <Badge className="bg-indigo-600 text-white rounded-full px-4 py-1 text-xs font-black uppercase tracking-wider border-none">
                    À faire maintenant
                  </Badge>
                ) : isLocked ? (
                  <Badge className="bg-zinc-200 text-zinc-600 rounded-full px-4 py-1 text-xs font-black uppercase tracking-wider border-none">
                    Bientôt
                  </Badge>
                ) : (
                  <Badge className={`${theme.bg} ${theme.text} rounded-full px-4 py-1 text-xs font-black uppercase tracking-wider border-none`}>
                    {category}
                  </Badge>
                )}

                <div className={`flex flex-wrap items-center justify-center md:justify-start gap-4 ${isCompleted ? 'opacity-70' : ''}`}>
                  <Badge variant="secondary" className={`rounded-full px-4 py-1 text-xs font-black uppercase tracking-wider border-none ${difficultyColor}`}>
                    {difficulty}
                  </Badge>
              ...
              <h3 className={`text-lg md:text-xl font-black tracking-tight text-zinc-900 ${isLocked ? '' : theme.hoverText} transition-colors`}>
                {mainTitle}
              </h3>
            ...
            <div className="shrink-0 w-full md:w-auto">
              {!isLocked && (
                <Button className={`w-full md:w-48 h-14 rounded-2xl font-black uppercase tracking-widest text-sm gap-2 transition-all shadow-lg ${theme.button} ${theme.shadow}`}>
