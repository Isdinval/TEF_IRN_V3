  ...
  return (
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Popup className="fixed inset-y-0 left-0 z-[120] w-[85vw] max-w-xs h-full shadow-xl md:hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:slide-out-to-left-full data-[state=open]:slide-in-from-left-full duration-200">
          <DialogPrimitive.Close
            aria-label="Fermer le menu"
            className="absolute right-3 top-3 z-10 flex h-11 w-11 items-center justify-center rounded-full bg-white/90 text-zinc-500 shadow-sm hover:bg-zinc-100 hover:text-zinc-900"
          >
            <X className="h-5 w-5" />
          </DialogPrimitive.Close>
