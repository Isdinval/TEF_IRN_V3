...
export function QuestionNavigator() {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border-t md:border-t-0 md:border-l p-6 w-full md:w-80 flex flex-col gap-6 overflow-y-auto max-h-[40vh] md:max-h-none">
      <div className="flex items-center justify-between">
        <h3 className="font-black text-[#002654] dark:text-white">Navigation</h3>
        ...
        {questions.map((q, i) => {
          ...
          return (
            <button
              ...
              className={cn(
                "h-10 w-full rounded-2xl text-sm font-black transition-all border-2",
                isCurrent
                  ? "border-[#002654] bg-[#002654] text-white shadow-lg"
                  : isAnswered
                    ? "border-emerald-500 bg-emerald-50 text-emerald-700"
                    : "border-slate-100 dark:border-white/10 bg-slate-50 dark:bg-white/5 dark:bg-brand-dark text-slate-400 hover:border-slate-300"
              )}
            >
              {i + 1}
            </button>
