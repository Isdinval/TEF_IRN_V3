...
- **XP/série de la pratique libre CE/CO** (depuis le 2026-09-22) : jusque-là, un sujet CE/CO terminé n'ajoutait aucun XP, ne comptait pas pour la série de jours et n'émettait pas `exercise_completed` — invisible pour le compteur XP du jour du dashboard (RPC `get_dashboard_data`, `SUM(score)` sur `exercise_attempts` uniquement) et pour les funnels PostHog "premier exercice". `api/comprehension/complete` appelle désormais `awardXpAndStreak()` (`src/lib/xp-streak.ts`, factorisé depuis `api/exercise-complete` — même logique de série, écriture par le client admin car `total_xp`/`streak_count` sont protégées par trigger) avec `xpGain = % de bonnes réponses du sujet`, et émet `exercise_completed` avec la propriété `exercise_type: 'ce' | 'co'`. Aucune insertion dans `exercise_attempts` (FK `exercise_id -> exercises.id` incompatible avec les sujets CE/CO) : l'écriture se fait directement sur `profiles`. Pas de garde-fou anti-farming, comportement identique à celui des exercices vocab/qcm/trous (déjà sans dédoublonnage).
...
- **`hasFullExam`** (Essentiel/Premium/Super Premium ; depuis le 2026-09-22) : verrou dur sur le simulateur d'examen blanc (`/tef-irn/exam`), mais Gratuit n'est **pas** totalement exclu — il a un essai à vie de **1 section CE + 1 section CO**, tous examens confondus (`src/lib/exam-quota.ts`, dérivé de `exam_ce_co_attempts` : la présence d'au moins une tentative pour la section suffit à prouver l'essai consommé, aucun compteur dédié). EE (déjà gérée par `hasExamWritingCorrection`) et EO (déjà gérée par `hasOralCoach`) de l'examen blanc ne sont pas concernées par ce droit.

### Où c'est appliqué
- **Backend** : `api/coach/chat` (403 direct, Gratuit uniquement), `api/oral/session` + `api/oral/analyze` (403 via `hasOralCoach`), `api/writing/correct` (403 via `hasExamWritingCorrection`, uniquement si `context === 'exam'`), `api/exam/ce-co-complete` (403 via `hasFullExam`/essai CE/CO consommé — source de vérité).
- **Frontend** (écran verrouillé + CTA `/tef-irn/pricing`, redondant avec le verrou serveur mais évite une UX cassée) : `coach/page.tsx`, `oral/page.tsx`, `SpeakingSession.tsx` (section EO de l'examen blanc, aperçu verrouillé plutôt qu'écran plein), `SectionTransition.tsx` (briefing avant une section CE/CO de l'examen blanc, écran `ExamTrialBlocked`).
...
- **Essai CE/CO du simulateur d'examen blanc** : `api/exam/ce-co-complete` (403 avant toute correction si l'essai à vie de la section est déjà consommé — source de vérité), `api/exam/check` (contrôle en lecture seule à l'ouverture d'une section, appelé par `SectionTransition.tsx` avant le bouton "Commencer l'épreuve"). Événements PostHog : `exam_trial_blocked` (`source: 'check'` ou `'complete'`), `exam_paywall_cta_clicked`.
