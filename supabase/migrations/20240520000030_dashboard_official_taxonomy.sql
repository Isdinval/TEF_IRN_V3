-- Refonte de competency_radar vers la taxonomie officielle TEF IRN (CE/EE/EO)
--
-- Contexte : l'ancien competency_radar utilisait 5 catégories maison
-- (GRAMMAIRE/RÉDACTION/ÉCRIT/ORAL/PARLER) qui ne correspondent pas aux 4
-- compétences officielles évaluées par le TEF IRN (CO/CE/EO/EE), et dont
-- deux (ORAL, PARLER) étaient à 0% en permanence car exercises.type='oral'
-- et 'reformulage' n'ont jamais eu aucun contenu créé.
--
-- Nouvelle logique :
--   CE (Compréhension Écrite) = exercises.type IN ('qcm','trous',
--       'qcm_centre_entrainement') — le TEF IRN inclut une sous-partie
--       "maîtrise des structures de la langue" dans l'épreuve CE.
--   EE (Expression Écrite)    = exercises.type = 'ecrit' (inchangé)
--   EO (Expression Orale)     = oral_session_results.overall_score,
--       jamais interrogé par le dashboard jusqu'ici alors que la donnée
--       existe réellement (sessions du Coach oral sur /tef-irn/oral).
--   CO (Compréhension Orale)  = aucune source de données dans l'app —
--       volontairement absent d'ici, géré côté frontend comme un état
--       "Bientôt disponible" plutôt qu'un faux 0%.
--
-- Une compétence sans la moindre tentative n'apparaît pas dans le tableau
-- (COUNT = 0 exclu), plutôt que de s'afficher comme 0% de réussite.
...
BEGIN
    ...
    -- Competency radar : taxonomie officielle TEF IRN, uniquement les
    -- compétences avec au moins une tentative réelle (pas de faux 0%)
