-- Fix (item 1 du plan "Refonte recommandation erreur -> tag -> ressource") :
-- get_dashboard_data() ne remontait pas r.category / r.sub_category dans
-- v_recommendations, alors que ces colonnes existent sur `recommendations`
-- depuis la migration 20260720000001.
--
-- Impact du bug en prod :
-- - ActionPlanCard::sameIssue() comparait toujours `undefined === wp.category`
--   (jamais vrai) -> une recommandation deja generee pour un point faible
--   reapparaissait quand meme dans le bloc "En attente d'une action ciblee"
--   (doublon d'affichage entre RecommendationCard et le bloc orphelin).
-- - RecommendationCard recevait `category=undefined` -> son bouton
--   "Commencer maintenant" pour un type exercise/review retombait sur
--   /tef-irn/practice SANS AUCUN filtre, meme pas la categorie.
--
-- Seul changement : ajout de r.category, r.sub_category dans le SELECT qui
-- construit v_recommendations. Aucun autre bloc de la fonction n'est modifie.
--
-- Teste avant livraison (transaction BEGIN/CREATE OR REPLACE/SELECT/ROLLBACK) :
-- la sous-requete v_recommendations remonte bien category/sub_category sur les
-- 4 recommandations pending existantes en prod, aucune regression observee.
...
BEGIN
    ...
    -- FIX (item 1) : ajout de r.category, r.sub_category ci-dessous, seul
    -- changement de cette migration par rapport a la version precedente.
