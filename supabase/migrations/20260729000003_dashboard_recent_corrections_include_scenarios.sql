-- Point 2 (suite improve_coach_EE) : le widget dashboard "Corrections récentes" (recent_corrections
-- dans get_dashboard_data) ne lisait que exercise_attempts/exercises (pool SRS). Il ignorait
-- writing_scenario_attempts (examens blancs catalogue, cf. migration 20260729000001).
--
-- Normalisation des deux sources vers la même forme JSON attendue par
-- src/components/features/dashboard/new/RecentCorrectionsList.tsx :
--   - type forcé à 'examen_blanc' pour les scénarios -> badge distinct côté front
--     (voir RecentCorrectionsList.tsx : getTypeBadgeLabel)
--   - category = section (A/B) faute de colonne category sur writing_scenario_attempts
--   - instructions récupérées via jointure sur writing_exam_scenarios.sujet (LEFT JOIN,
--     scenario_id étant ON DELETE SET NULL : fallback texte si le scénario a été supprimé)
--   - knowledge_references dérivé directement des type_erreur stockés dans errors (jsonb),
--     pas de dépendance à ai_feedback (qui ne concerne que le pool SRS)
--
-- LIMIT déplacé au niveau du SELECT englobant pour un tri chronologique global sur les deux
-- sources (avant : LIMIT 5 appliqué uniquement au pool SRS).
--
-- Scope volontairement limité au widget dashboard. La page complète /tef-irn/correction
-- (historique, filtres, export) reste SRS-only pour l'instant (cf. point 3, non traité ici :
-- elle ignore encore ?id= de toute façon, comportement préexistant et indépendant).
