-- Item 11 du plan "Refonte recommandation erreur -> tag -> ressource" :
-- v_weak_points triait uniquement par frequency DESC, last_seen_at DESC --
-- un point faible ancien mais frequent pouvait donc monopoliser le widget
-- "Points faibles" indefiniment, meme si l'utilisateur n'a plus fait cette
-- erreur depuis des semaines et progresse sur d'autres points plus recents.
--
-- Decision validee : depriorisation, pas suppression -- l'historique reste
-- en base (frequency n'est modifiee que par trackUserError/resolveUserError,
-- jamais par cette requete de lecture). Un point faible dont last_seen_at
-- depasse 30 jours passe simplement apres tous les points faibles recents
-- dans le tri, quelle que soit sa frequency.
--
-- Seul changement : ORDER BY de la sous-requete v_weak_points. Aucun autre
-- bloc de la fonction n'est modifie (reprend le fix item 1 deja applique).
--
-- Teste avant livraison : clause verifiee sur les user_errors reels du
-- projet (aucune ligne stale actuellement, ordre inchange sur ces donnees --
-- comportement visible seulement une fois qu'un point faible depasse 30
-- jours sans nouvelle occurrence).
