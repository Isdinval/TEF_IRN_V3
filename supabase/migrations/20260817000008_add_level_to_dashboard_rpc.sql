-- Suite du fix critique découvert en test P0 (voir migration
-- 20260817000007) : la colonne level existe maintenant sur user_errors et
-- recommendations, mais get_dashboard_data() ne la remontait pas. Cette
-- migration :
--
-- 1. Ajoute `level` à v_weak_points (lu depuis user_errors) et
--    v_recommendations (lu depuis recommendations), pour que le patch
--    applicatif associé puisse propager le bon niveau dans le routing vers
--    /tef-irn/practice.
-- 2. Ajoute `level` à l'objet `exercise` de chacun des 4 blocs de
--    v_recent_corrections (exercices ciblés, EE, EO, CE/CO) -- nécessaire
--    pour que les badges de notions cliquables dans "Corrections récentes"
--    transmettent eux aussi le bon niveau (RecentCorrectionsList.tsx, patch
--    applicatif associé). Pour le bloc CE/CO, level vient de exams.level
--    via une jointure supplémentaire sur exam_questions.exam_id -- ni
--    exam_ce_co_attempts ni exam_questions n'ont leur propre colonne level,
--    seul l'examen parent en a une.
--
-- Testé avant livraison (transaction BEGIN/ALTER/CREATE OR REPLACE/
-- ROLLBACK) : la fonction complète compile sans erreur.
...
BEGIN
    ...
    FROM (
        SELECT r.id, r.type, r.reference_id, r.reason, r.status, r.created_at, r.category, r.sub_category, r.level, l.slug
