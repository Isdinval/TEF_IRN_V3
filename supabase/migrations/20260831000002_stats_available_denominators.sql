-- Fix "dashboard-mastery-clarity" -- item 1/plan.
--
-- Problème corrigé : les 3 blocs de stats "maîtrisé" (vocab, QCM, trous)
-- retournaient un total sans dénominateur. Côté front, les barres de
-- répartition par niveau étaient calculées en % du TOTAL MAÎTRISÉ
-- (ex: count_A1 / total_maîtrisé), pas du catalogue réel -- ce qui donnait
-- des barres artificiellement pleines (~100% cumulé) quel que soit le
-- niveau réel de progression. Ce fichier ajoute le vrai dénominateur
-- (`total_available` / `levels_available`), calculé en direct sur le
-- catalogue global (choix validé avec Olivier -- pas de restriction à la
-- plage current_level -> goal_level de l'utilisateur, pour rester simple).
--
-- get_qcm_stats() et get_trous_stats() : CREATE OR REPLACE intégral.
-- get_dashboard_data() : CREATE OR REPLACE intégral également (Postgres ne
-- permet pas de patcher partiellement un corps de fonction plpgsql), mais
-- SEUL le bloc vocab_stats change -- tout le reste est repris à l'identique
-- de la définition en production (vérifié par diff avant livraison, et par
-- comparaison des clés de l'objet JSON retourné en dry-run).
...
BEGIN
    ...
    -- Dénominateur "total possible" -- catalogue global, calculé en direct
    -- (jamais de valeur en dur) pour rester exact quand le catalogue grandit.
    SELECT
        count(*),
        count(*) FILTER (WHERE level = 'A1'),
        count(*) FILTER (WHERE level = 'A2'),
        count(*) FILTER (WHERE level = 'B1'),
        count(*) FILTER (WHERE level = 'B2')
    INTO v_total_available, v_avail_a1, v_avail_a2, v_avail_b1, v_avail_b2
    FROM public.exercises
    WHERE type IN ('qcm', 'association', 'qcm_centre_entrainement');
    ...
    RETURN jsonb_build_object(
        ...
        'levels_available', jsonb_build_object(
            'A1', COALESCE(v_avail_a1, 0), 'A2', COALESCE(v_avail_a2, 0),
            'B1', COALESCE(v_avail_b1, 0), 'B2', COALESCE(v_avail_b2, 0)
        ),
