-- Fix bug pre-existant (trouve en amont de l'item "Cap examen" du plan
-- "cards dashboard") : parcours_overview n'etait construit par AUCUNE
-- migration -- get_dashboard_data() ne retournait jamais cette cle, donc
-- data.parcours_overview valait toujours undefined cote front, et
-- ParcoursOverviewCard (deja integree dans l'onglet "Ma progression")
-- faisait systematiquement `if (!overview || overview.total === 0) return
-- null` -- la card ne s'est donc jamais affichee en production.
--
-- CREATE OR REPLACE integral (Postgres ne permet pas de patcher
-- partiellement un corps de fonction plpgsql), mais SEUL le bloc
-- parcours_overview est nouveau -- tout le reste est repris a l'identique
-- de la version en production (20260831000002_stats_available_denominators.sql).
...
DECLARE
    ...
    v_parcours_overview JSONB;
    v_parcours_total INTEGER;
    v_parcours_completed INTEGER;
    v_parcours_in_progress INTEGER;
...
BEGIN
    ...
    FROM (
        ...
        FROM (
            ...
            JOIN public.exams exm ON exm.id = eq.exam_id
            WHERE a.user_id = v_user_id
            GROUP BY a.user_id, eq.exam_id, a.section, a.created_at, exm.level
        ) ceco WHERE rn <= 5
    ) t;
    ...
    FROM (
        SELECT category, sub_category, frequency, last_seen_at, source_label, level
    ...
    -- Dénominateur "total possible" pour vocab_stats -- catalogue global
    -- (choix validé avec Olivier), calculé en direct pour rester exact
    -- quand le catalogue grandit. Seule partie modifiée de cette fonction
    -- (fix "dashboard-mastery-clarity", item 1/plan) -- tout le reste est
    -- repris à l'identique de la version en production.
    SELECT jsonb_build_object(
        'total', count(*),
        'levels', jsonb_build_object('A1', count(*) FILTER (WHERE level = 'A1'), 'A2', count(*) FILTER (WHERE level = 'A2'), 'B1', count(*) FILTER (WHERE level = 'B1'), 'B2', count(*) FILTER (WHERE level = 'B2'))
    ) INTO v_vocab_available
    FROM public.vocabulary;

    SELECT jsonb_build_object(
        'total', count(*),
        'total_available', (v_vocab_available->>'total')::int,
        ...
        'levels_available', v_vocab_available->'levels',
    ...
    FROM (
        SELECT r.id, r.type, r.reference_id, r.reason, r.status, r.created_at, r.category, r.sub_category, r.level, r.is_reminder, l.slug
    ...
    -- Fix bug pre-existant : parcours_overview n'etait jamais calcule, donc
    -- toujours undefined cote front -> ParcoursOverviewCard ne s'affichait
    -- jamais (return null sur overview.total === 0). Meme perimetre catalogue
    -- global que get_qcm_stats/get_trous_stats/vocab_stats (pas de filtre sur
    -- current_level/goal_level, pour rester simple).
    SELECT
        count(*),
        count(*) FILTER (WHERE COALESCE(upp.progress_percentage, 0) = 100),
        count(*) FILTER (WHERE COALESCE(upp.progress_percentage, 0) > 0 AND COALESCE(upp.progress_percentage, 0) < 100)
    INTO v_parcours_total, v_parcours_completed, v_parcours_in_progress
    FROM public.parcours p
    LEFT JOIN public.user_parcours_progress upp ON upp.parcours_id = p.id AND upp.user_id = v_user_id;

    v_parcours_overview := jsonb_build_object(
        'total', COALESCE(v_parcours_total, 0),
        'completed', COALESCE(v_parcours_completed, 0),
        'in_progress', COALESCE(v_parcours_in_progress, 0),
        'not_started', COALESCE(v_parcours_total, 0) - COALESCE(v_parcours_completed, 0) - COALESCE(v_parcours_in_progress, 0)
    );
    ...
    RETURN jsonb_build_object(
        ...
        'parcours_overview', v_parcours_overview
    );
END;
$function$;
