-- Ajout de xp_last_7_days dans get_dashboard_data
--
-- Contexte : XPChart (src/components/features/dashboard/new/XPChart.tsx) affichait
-- des données factices (dummyData codées en dur) au lieu de l'activité réelle de
-- l'utilisateur. Cette migration ajoute un historique XP sur 7 jours glissants
-- (aujourd'hui inclus), calculé sur exercise_attempts.score, avec un jour à 0 XP
-- pour les jours sans tentative (pas de faux zéro pour un jour futur : la série
-- s'arrête à CURRENT_DATE).
...
BEGIN
    ...
    -- 2b. XP des 7 derniers jours glissants (pour XPChart)
    ...
    -- TODO(dashboard-phase-0.6): league_stats reste statique (rank/total_members/league_name
    -- codés en dur), non consommé par le frontend aujourd'hui.
