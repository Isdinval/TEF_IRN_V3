-- Le temps d'étude par tentative (exercise_attempts.study_time_minutes)
-- arrondit quasi toujours à 0 pour un QCM rapide (quelques secondes).
-- On passe à un suivi global de connexion : le client ping toutes les
-- 60s pendant que l'utilisateur est actif dans l'app (leçon, exercice,
-- examen, coach oral, etc.), peu importe la page.
...
AS $function$
BEGIN
  INSERT INTO public.study_activity (user_id, activity_date, minutes)
  VALUES (auth.uid(), CURRENT_DATE, 1)
  ON CONFLICT (user_id, activity_date)
  DO UPDATE SET minutes = public.study_activity.minutes + 1, updated_at = NOW();
END;
$function$;

-- Le dashboard lit désormais study_activity pour "Temps d'étude" au lieu
-- de sommer exercise_attempts.study_time_minutes (conservé par ailleurs
-- pour l'affichage par tentative dans RecentCorrectionsList/CorrectionCard).
