-- Phase 2 (improve_coach_EO) - items 2/3 : oral_session_results était jusqu'ici write-only,
-- jamais relu par get_dashboard_data (le radar EO fonctionnait déjà via competency_radar,
-- voir 20260729000005, mais recent_corrections et sub_competencies ignoraient l'oral).
--
-- 1. recent_corrections : ajoute une branche oral_session_results, même format que la
--    branche writing_scenario_attempts (type = 'entretien_oral' pour que
--    RecentCorrectionsList route correctement vers /tef-irn/oral/history au lieu de
--    /tef-irn/correction).
--
-- 2. sub_competencies : contrairement à l'EE (qui n'a qu'un score global -> on ne peut
--    déduire les catégories faibles que via les erreurs détectées), l'oral a déjà 5 scores
--    par critère indépendants -> pas besoin de proxy, chaque critère alimente directement
--    son propre point de donnée. 'Grammaire' et 'Vocabulaire' réutilisent volontairement
--    les mêmes labels que l'EE/les exercices SRS (même principe de casse-alignée déjà
--    documenté dans 20260729000005) pour que les points faibles se cumulent dans le même
--    bucket toutes compétences confondues. Les 3 autres critères oraux (pertinence,
--    cohérence, fluidité) n'ont pas d'équivalent ailleurs : nouveaux buckets dédiés.
...
BEGIN
    ...
    FROM (
        SELECT label, ROUND(AVG(score)) as score, COUNT(*) as count
        FROM (
            SELECT e.category as label, ea.score as score
            FROM public.exercise_attempts ea JOIN public.exercises e ON ea.exercise_id = e.id
            WHERE ea.user_id = v_user_id AND ea.score IS NOT NULL AND e.category IS NOT NULL

            UNION ALL

            SELECT DISTINCT INITCAP(err->>'type_erreur') as label, wsa.overall_score::double precision as score
            FROM public.writing_scenario_attempts wsa
            ...
            SELECT 'Grammaire' as label, (osr.scores->>'correction_grammaticale')::double precision as score
            FROM public.oral_session_results osr
            WHERE osr.user_id = v_user_id AND osr.scores ? 'correction_grammaticale'

            UNION ALL

            SELECT 'Vocabulaire', (osr.scores->>'etendue_et_precision_du_vocabulaire')::double precision
            FROM public.oral_session_results osr
            WHERE osr.user_id = v_user_id AND osr.scores ? 'etendue_et_precision_du_vocabulaire'

            UNION ALL

            SELECT 'Pertinence & adéquation au sujet', (osr.scores->>'pertinence_et_adequation_au_sujet')::double precision
            FROM public.oral_session_results osr
            WHERE osr.user_id = v_user_id AND osr.scores ? 'pertinence_et_adequation_au_sujet'

            UNION ALL

            SELECT 'Cohérence & interaction', (osr.scores->>'coherence_et_interaction')::double precision
            FROM public.oral_session_results osr
            WHERE osr.user_id = v_user_id AND osr.scores ? 'coherence_et_interaction'

            UNION ALL

            SELECT 'Aisance & fluidité', (osr.scores->>'aisance_et_fluidite')::double precision
            FROM public.oral_session_results osr
            WHERE osr.user_id = v_user_id AND osr.scores ? 'aisance_et_fluidite'
        ) combined
        GROUP BY label ORDER BY score DESC
    ) t;
