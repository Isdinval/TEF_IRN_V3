-- Le score "EE" (Expression Écrite) du radar de compétences (get_dashboard_data) ne se basait
-- que sur exercise_attempts/exercises (type='ecrit', pool SRS). Il ignorait totalement les
-- tentatives d'examen blanc (writing_scenario_attempts, cf. migration 20260729000001), pourtant
-- le mode d'usage principal de /tef-irn/writing. Seul le sous-bloc EE change, tout le reste de
-- la fonction est identique (CREATE OR REPLACE réécrit l'ensemble par contrainte Postgres).
...
BEGIN
    ...
    -- Bloc "EE" modifié (2026-07) : le score Expression Écrite du radar de compétences
    -- doit refléter à la fois les exercices SRS (exercises type='ecrit') ET les examens
    -- blancs du catalogue (writing_scenario_attempts), sinon un utilisateur qui pratique
    -- surtout via le catalogue voit un score EE quasi vide.
