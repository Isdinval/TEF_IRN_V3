-- Bug de casse : `ROUND(sum / count) as A` non quoté est replié en
-- minuscule par Postgres ("a"), alors que le frontend (Recharts
-- dataKey="A", StatsOverview curr.A) attend une clé "A" majuscule.
-- Résultat : competency_radar contenait bien les scores mais sous une
-- clé jamais lue côté client -> radar et Score Moyen à 0 en permanence,
-- ce depuis la toute première version de cette fonction.
