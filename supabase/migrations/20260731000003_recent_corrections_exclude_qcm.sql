-- Suite du découpage de "Corrections récentes" en 3 blocs (Examen blanc /
-- Écrit / Oral) : le bloc "Écrit" ne doit plus inclure le QCM (types 'qcm',
-- 'trous', 'qcm_centre_entrainement' de exercise_attempts), uniquement les
-- exercices de type 'ecrit' (expression écrite).
--
-- Un simple filtre côté frontend aurait laissé un problème identique à celui
-- corrigé en 20260731000002 : les 5 lignes remontées par la branche
-- exercise_attempts (ROW_NUMBER() LIMIT 5) peuvent être en grande partie du
-- QCM récemment pratiqué, ce qui viderait le bloc "Écrit" même si
-- l'utilisateur a des rédactions plus anciennes non montrées. On exclut donc
-- directement le QCM de cette branche de la requête : les 5 slots de ce
-- bloc ne sont désormais jamais partagés avec le QCM, qui n'est plus
-- remonté du tout par recent_corrections (il n'apparaît dans aucun des 3
-- blocs du dashboard).
...
BEGIN
    ...
    FROM (
        ...
        FROM (
            ...
            WHERE ea.user_id = v_user_id AND e.type = 'ecrit'
        ) ex WHERE rn <= 5
