-- Deux changements sur get_dashboard_data() :
--
-- 1. Expose profiles.target_exam_date (collecté à l'onboarding depuis
--    20260720000004 mais jamais renvoyé par la RPC ni affiché nulle part) --
--    nécessaire au compte à rebours "J-N avant l'examen" du dashboard.
--
-- 2. recent_corrections passait par un unique `ORDER BY created_at DESC
--    LIMIT 5` appliqué à l'UNION ALL des 3 sources (exercice, examen blanc,
--    oral) confondues : un utilisateur actif à l'écrit mais jamais à l'oral
--    ne voyait alors jamais aucune session orale, pas parce qu'il n'en a pas
--    fait, mais parce que la requête ne remontait jamais plus de 5 lignes au
--    total. Le nouveau découpage du dashboard en 3 blocs (Examen blanc /
--    Écrit-QCM / Oral) exige une limite par catégorie : chaque branche de
--    l'UNION a maintenant son propre ROW_NUMBER() OVER (ORDER BY created_at
--    DESC), filtré à 5, avant l'agrégation finale (jusqu'à 15 lignes au lieu
--    de 5). Le frontend fait le tri par `exercise.type`.
