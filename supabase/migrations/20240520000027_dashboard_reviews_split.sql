-- Séparation de reviews_count en vocab_reviews_due / exercise_reviews_due
--
-- Contexte : reviews_count (user_reviews + user_vocabulary_reviews confondus)
-- ne peut pas alimenter un CTA unique fiable côté dashboard : seul le
-- vocabulaire (/tef-irn/vocab) a une vraie page qui priorise les révisions
-- dues au sens SRS (next_review_at <= now). Les révisions d'exercices
-- générales (user_reviews) ne sont consommées que via /parcours/[slug] ou
-- /lessons/[slug]/complete, qui nécessitent un parcours précis — pas de
-- point d'entrée générique. On expose donc les deux compteurs séparément,
-- reviews_count restant disponible tel quel pour compatibilité.
