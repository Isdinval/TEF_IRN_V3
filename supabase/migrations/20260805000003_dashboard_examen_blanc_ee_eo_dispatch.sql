-- Item 7/10 du plan de correction du dashboard (analyse détaillée) :
--
-- Jusqu'ici, toute ligne de writing_scenario_attempts était taguée 'examen_blanc'
-- et toute ligne de oral_session_results 'entretien_oral', sans distinction. Grâce
-- à la colonne context ajoutée en 20260805000001 (+ context='exam' désormais posé
-- par ExamContext/SpeakingSession en exam, items 5-6), on peut enfin répartir
-- correctement :
--   - context='exam'       -> type 'examen_blanc' (que ce soit une EE ou une EO)
--   - context='standalone' -> type 'ecrit' (EE, page Rédaction) ou 'entretien_oral'
--                              (EO, page Expression Orale), comme avant
--
-- Un nouveau champ `skill` ('EE' | 'EO') est ajouté à chaque ligne, indépendant du
-- `type` : le bloc "Examen blanc" du dashboard va désormais contenir à la fois des
-- EE et des EO, `type` seul ne suffit plus pour savoir vers quelle page rediriger
-- au clic (correction écrite vs historique oral) -- le frontend (item 9) doit
-- utiliser `skill`, pas `type`, pour ce routing.
--
-- Libellé des cartes EE d'examen blanc (option B validée) : jointure sur
-- exam_questions via exam_question_id (rempli uniquement pour context='exam',
-- voir 20260805000002) pour afficher le vrai sujet plutôt qu'un texte générique.
