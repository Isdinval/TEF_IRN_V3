-- Remplacement du stub league_stats par une vraie logique
--
-- Contexte : league_stats était codé en dur (rank=4, total_members=25,
-- league_name='Diamant'), non consommé par le frontend. La table `leagues`
-- (Bronze/Argent/Or/Platine/Diamant, par seuil de min_xp) existe déjà et
-- profiles.total_xp est déjà calculé — la ligue d'un utilisateur est donc
-- dérivable directement, sans table d'affectation séparée.
--
-- Logique : la ligue de l'utilisateur = la ligue avec le plus grand min_xp
-- tel que min_xp <= total_xp. Le rang et le nombre de membres sont calculés
-- parmi les profils dont le total_xp tombe dans la même tranche
-- [min_xp de la ligue, min_xp de la ligue suivante[.
...
BEGIN
    ...
    -- League Stats (réel, calculé à partir de public.leagues et profiles.total_xp)
