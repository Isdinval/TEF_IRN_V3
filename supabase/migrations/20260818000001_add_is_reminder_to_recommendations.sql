-- Item 18 du plan "Refonte recommandation erreur -> tag -> ressource" :
-- la carte "Maîtriser une nouvelle leçon" affiche toujours le même titre,
-- que la leçon n'ait jamais été lue OU qu'il s'agisse d'un rappel après une
-- erreur persistante malgré une lecture déjà faite (item 10) -- seul le
-- texte `reason` change entre les deux cas, jamais le titre de la carte.
-- Signalé pendant la session de test P0 : "ce n'est plus une nouvelle
-- leçon !" pour le cas rappel.
--
-- Colonne dédiée plutôt qu'une dérivation côté client (ex. sur frequency) :
-- alreadyRead && isPersistent n'est pas déductible de façon fiable à partir
-- des seules données déjà exposées au front (frequency >= 2 seul ne suffit
-- pas à distinguer "jamais lue" de "rappel", cf. analyse).

ALTER TABLE public.recommendations ADD COLUMN IF NOT EXISTS is_reminder BOOLEAN NOT NULL DEFAULT false;
