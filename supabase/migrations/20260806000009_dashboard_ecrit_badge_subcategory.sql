-- Suite de l'item 10.12 du plan de robustification des étiquettes : alignement
-- demandé par Olivier après livraison de la sous-catégorie Écrit.
--
-- Les badges de notions de l'Analyse détaillée (bloc Écrit/Examen blanc) n'affichaient
-- que la catégorie ("Grammaire"), jamais la sous-catégorie -- contrairement au rappel
-- de la card "action ciblée" (item 10.3), qui l'affiche déjà. Depuis l'item 10.12,
-- wsa.errors contient une vraie sous_categorie par erreur (taxonomie officielle) :
-- on l'utilise ici pour afficher "Grammaire (Comparatifs)" plutôt que "Grammaire" seul.
--
-- Dédoublonnage par paire (type_erreur, sous_categorie) plutôt que par type_erreur
-- seul, pour ne pas perdre deux notions différentes de la même catégorie dans un
-- même texte (ex. Grammaire/Comparatifs ET Grammaire/Accord des adjectifs).
--
-- Le clic sur le badge (RecentCorrectionsList.tsx) doit continuer de fonctionner :
-- il extrait la catégorie avant la parenthèse pour construire le lien vers
-- /tef-irn/practice, donc le format "Catégorie (Sous-catégorie)" reste compatible
-- sans changement du mécanisme de clic existant.
