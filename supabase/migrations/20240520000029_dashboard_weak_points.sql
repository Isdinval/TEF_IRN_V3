-- Ajout de weak_points dans get_dashboard_data
--
-- Contexte : COACH_GUIDE.md documente un outil `get_weak_points` pour le
-- coach-chat Edge Function, qui n'existe pas dans le code réel (seul
-- generate_exercise y est implémenté). Plutôt que de dépendre de cet outil
-- absent, on interroge directement user_errors (déjà la source fiable et
-- agrégée utilisée par recommendation-engine.ts) pour alimenter un widget
-- "Points à travailler" sur le dashboard, avec CTA vers le Coach IA.
...
BEGIN
    ...
    -- Points faibles : top 3 catégories d'erreurs récurrentes (user_errors)
