-- Suite improve_coach_EO : retour utilisateur après test réel en preview -- la heatmap
-- "Analyse des sous-compétences" mélangeait 5 catégories ÉCRIT (Orthographe/Syntaxe/
-- Grammaire/Conjugaison/Vocabulaire) et 3 catégories ORALES (Pertinence/Cohérence/Aisance)
-- dans une seule liste plate, en fusionnant en plus Grammaire/Vocabulaire EE+EO dans le
-- même point de donnée (voir 20260729000009) : illisible, impossible de distinguer visuellement
-- ce qui relève de l'écrit et de l'oral.
--
-- Changement : ajoute un champ 'group_name' ('ÉCRIT' / 'ORAL') à chaque ligne, et Grammaire/
-- Vocabulaire ne sont PLUS fusionnés entre EE et EO -- chaque groupe a maintenant ses 5
-- (ÉCRIT) ou 5 (ORAL) points de données propres, GROUP BY (label, group_name) au lieu de
-- GROUP BY label seul. Le frontend (SubSkillHeatmap.tsx) affiche deux sections séparées.
--
-- Item repris de 20260729000009 sans changement : recent_corrections (branche oral_session_results).
