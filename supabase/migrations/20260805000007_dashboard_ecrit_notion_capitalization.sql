-- Item 10.5 du plan de correction du dashboard (analyse détaillée) :
--
-- Les badges de notions des cartes Écrit/Examen blanc (Analyse détaillée)
-- affichaient le libellé brut de l'erreur détectée (ex. "grammaire", en
-- minuscule), alors que partout ailleurs dans le dashboard (catégories de
-- "En attente d'une action ciblée", filtres de la page d'entraînement) le
-- même mot s'affiche capitalisé ("Grammaire"). Applique INITCAP, déjà utilisé
-- pour ce même besoin un peu plus bas dans cette fonction (sub_competencies).
            ...
            SELECT wsa.id, wsa.created_at, wsa.overall_score::double precision as score, wsa.study_time_minutes,
                ...
                jsonb_build_object(
                    ...
                    'knowledge_references', COALESCE((
                        SELECT jsonb_agg(DISTINCT INITCAP(err->>'type_erreur'))
