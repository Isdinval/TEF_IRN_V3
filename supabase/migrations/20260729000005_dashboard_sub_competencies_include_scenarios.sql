-- Point 4 (suite improve_coach_EE) : la heatmap "Analyse des sous-competences" (sub_competencies
-- dans get_dashboard_data) ne se basait que sur exercises.category (pool SRS), ignorant les
-- erreurs detectees dans les examens blancs (writing_scenario_attempts.errors).
--
-- Choix de conception : un examen blanc n'a qu'un score global, pas de score par categorie
-- comme les exercices SRS. On utilise donc le score global de l'examen comme point de donnee,
-- UNIQUEMENT pour les categories ou une erreur de ce type a ete detectee dans ce texte precis
-- (DISTINCT par tentative, pas par occurrence, pour ne pas sur-ponderer un texte avec 3 fois
-- la meme faute de grammaire). Aucun point de donnee ajoute pour les categories absentes du
-- texte : l'absence d'erreur de syntaxe dans un message court ne prouve pas la maitrise de la
-- syntaxe (meme logique epistemique qu'au point 1 pour user_errors / resolveUserError).
--
-- INITCAP(type_erreur) aligne la casse ('grammaire' -> 'Grammaire') sur celle deja utilisee par
-- exercises.category ('Grammaire', 'Conjugaison', ...), pour que les deux sources alimentent le
-- meme bucket au lieu de creer des doublons de categorie a casse differente.
